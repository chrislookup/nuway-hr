        ...
        {pages && pages.map((pg, pi) => (
          <section key={pi} style={{ marginTop: 16 }}>
            ...
            {(pg.blocks || []).map((b, bi) => {
              ...
              if (b.type === 'field') return <p key={bi} className="filled"><b>{b.label}:</b> {fd[b.name] || b.default || <span className="muted">—</span>}</p>
              if (b.type === 'ack') return (
                <p key={bi} className="filled">
                  {fd[b.name] ? '☑' : '☐'} {b.text}
                  {b.initials && <b> — initials: {fd[b.name + '_initials'] || '—'}</b>}
                </p>
              )
              return null
            })}
